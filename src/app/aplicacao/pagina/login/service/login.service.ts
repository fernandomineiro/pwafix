...
export class LoginService extends Http {
  ...
  public autenticarUsuario(usuario: UsuarioLogin): Observable<JwtResponse> {
    return this.http.post<JwtResponse>(`${this.api}/usuario`, usuario);
  }

  LoginItem(item): Observable<UsuarioLogin> {
    return this.http
      .post<UsuarioLogin>(this.api + '/api/auth/logar', JSON.stringify(item), this.httpOptions)
      .pipe(
        retry(2),
